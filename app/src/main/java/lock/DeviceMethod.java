...
public class DeviceMethod {
    private static final String PLEASE_ACTIVE_DEVICE_SAY="请先激活设备";
    ...
    public void onActivate() {
        ...
        if (!devicePolicyManager.isAdminActive(componentName)) {
            ...
            intent.putExtra(DevicePolicyManager.EXTRA_DEVICE_ADMIN,
                    componentName);
            intent.putExtra(DevicePolicyManager.EXTRA_ADD_EXPLANATION, "请点击激活按钮！");
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            mContext.startActivity(intent);
        }else {
            //Toast.makeText(mContext, "设备已经激活,请勿重复激活", Toast.LENGTH_SHORT).show();
            MyLog.i(PLEASE_ACTIVE_DEVICE_SAY);
        }
    }
