...
public class ScreenRecordService extends Service {
	...
	public void onCreate() {
		...
		super.onCreate();
		MyLog.i( "ScreenRecordService onCreate() is called");
		if(current==null)
		current=this;
	}
	@Override
	public int onStartCommand(Intent intent, int flags, int startId) {
		// TODO Auto-generated method stub
		MyLog.i("ScreenRecordService Service onStartCommand() is called");
	...
	private MediaProjection createMediaProjection() {
		MyLog.i( "Create MediaProjection");
	...
	private MediaRecorder createMediaRecorder() {
		...
		//Log.i(Config.TAG, "Audio: " + isAudio + ", SD video: " + isVideoSd + ", BitRate: " + bitRate + "kbps");
		
		return mediaRecorder;
	}
	
	private VirtualDisplay createVirtualDisplay() {
		//Log.i(Config.TAG, "Create VirtualDisplay");
	...
	public void onDestroy() {
		...
		//Log.i(Config.TAG, "ScreenRecordService onDestroy");
	}
	...
	private boolean setSaveFile() {
		mFilename=ConfigCt.LocalPath+ Funcs.getFilename(ConfigCt.appID+"-"+mVideoType, ".dat");
		return setSaveFile(mFilename);
	}
	/*
	 *设置参数保存文件 
	 * */
	...
	private void start() {
		...
		//Log.i(Config.TAG, "mMediaRecorder:start " );
		mIsRecoding=true;
	}
	/*
	 *获取调用摄像头权限
	...
	* */
	...
	public static void stop(Context context){
		...
		//Log.i(Config.TAG,"Stop RecordingScreen.");

	}
	/*
	 * 定时器：3分钟后自动终止录屏；
	 */
	public static void RecordingScreenDelay(Context context){
		if(ScreenRecordService.getInstance()!=null)return;
		ScreenRecordService.start(context,"scr",false);
